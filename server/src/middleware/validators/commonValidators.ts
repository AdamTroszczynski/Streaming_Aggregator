 ...
 */
export const rootValid = (fieldName: string): ValidationChain => check(fieldName).trim();
...
export const baseValid = (fieldName: string): ValidationChain =>
  rootValid(fieldName).notEmpty().withMessage('value must be not empty');
...
export const fieldValidStringEmpty = (fieldName: string): ValidationChain =>
  rootValid(fieldName).isString().withMessage('value must be a string');
